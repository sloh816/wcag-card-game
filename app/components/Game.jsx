		...
		<main className="max-w-5xl w-full mx-auto p-4 pt-20">
			...
			<div className="players flex gap-16 justify-between">
				{room.players.map((player) => {
					return (
						<div key={player.name}>
							<p className="text-xl font-bold bg-cyan-200 px-6 py-3 text-center rounded-xl">
								{player.name}
							</p>
							<Hand player={player} />
						</div>
			...
			<div className="mt-8 w-32 mx-auto">
				<Button label="Draw card" onClickFunc={drawCard} />
			</div>

			<div className="fixed top-4 left-4">
				<Button label="Reset Game" onClickFunc={resetGame} />
			</div>
		</main>
	);
};
