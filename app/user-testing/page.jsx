...
const Page = ({}) => {
	...
	const fetchDirectories = async () => {
		const directories = await userTesting.getDirectories();
		setDirectories(directories);
	};

	const fetchArticles = async (directory) => {
		const articles = await userTesting.getArticlesByDirectory(directory);
	...
	const updateQuery = async (newQueryValue, key) => {
		if (key === "directory") {
			...
			router.push(`/user-testing?directory=${newQueryValue}`, undefined, { shallow: true });
			...
			router.push(
				`/user-testing?directory=${directory}&article=${newQueryValue}`,
				undefined,
				{
					shallow: true
				}
			);
			const article = await userTesting.getArticleBySlug(newQueryValue);
	...
	useEffect(() => {
		...
		if (article) {
			userTesting.getArticleBySlug(article).then((article) => {
				setSelectedArticle(article);
			});
		}
